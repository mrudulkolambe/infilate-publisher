				...
				<div className='hidden lg:block left-position absolute top-20 px-10 py-6 Nunito w-10/12 h-calc-height overflow-scroll'>
					<div className='flex justify-between items-center'>
						...
						<div className='w-4/12 mx-2 h-40 rounded-lg shadow-lg bg-red-400 p-4 flex flex-col justify-around'>
							...
							<button disabled={user && user.kyc == 'Approved' ? true : false } onClick={() => {useRouter().push('/publisher-kyc')}} className='w-max text-white font-bold bg-gray-900 px-3 py-1 duration-200 rounded-lg hover:bg-gray-700'>Apply For KYC</button>
						</div>
					</div>
